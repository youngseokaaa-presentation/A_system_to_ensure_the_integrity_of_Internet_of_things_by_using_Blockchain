...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CYoungseokcoinAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/youngseokcoin/youngseokcoin/pull/10340, but that change has been
    // postponed until after https://github.com/youngseokcoin/youngseokcoin/pull/10244 to
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CYoungseokcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CYoungseokcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CYoungseokcoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CYoungseokcoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CYoungseokcoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CYoungseokcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CYoungseokcoinAddress(dest).ToString(), key);
}
