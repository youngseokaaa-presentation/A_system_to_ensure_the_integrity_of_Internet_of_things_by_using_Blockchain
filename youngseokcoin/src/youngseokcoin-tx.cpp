...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s youngseokcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  youngseokcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded youngseokcoin transaction") + "\n" +
              "  youngseokcoin-tx [options] -create [commands]   " + _("Create hex-encoded youngseokcoin transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CYoungseokcoinAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        ...
        CYoungseokcoinAddress redeemScriptAddr(scriptPubKey);
...
static void MutateTxAddOutMultiSig(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        ...
        // Get the address for the redeem script, then call
        // GetScriptForDestination() to construct a P2SH scriptPubKey.
        CYoungseokcoinAddress addr(scriptPubKey);
