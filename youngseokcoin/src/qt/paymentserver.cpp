...
const char* BIP71_MIMETYPE_PAYMENT = "application/youngseokcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/youngseokcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/youngseokcoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(YOUNGSEOKCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // youngseokcoin: URI
        {
            ...
            {
                CYoungseokcoinAddress address(r.address.toStdString());
...
//
// OSX-specific way of handling youngseokcoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in youngseokcoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(YOUNGSEOKCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // youngseokcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            {
                CYoungseokcoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CYoungseokcoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom youngseokcoin addresses are not supported
        ...
        // Youngseokcoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(YoungseokcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
