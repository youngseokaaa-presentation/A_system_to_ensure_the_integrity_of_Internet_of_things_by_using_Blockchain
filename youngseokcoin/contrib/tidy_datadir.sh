#!/bin/bash
# Copyright (c) 2013 The Youngseokcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Youngseokcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Youngseokcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Youngseokcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Youngseokcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Youngseokcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Youngseokcoin 0.8 datadir."
    ;;
esac
