...
The RPM spec file provided here is for Youngseokcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://youngseokcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Youngseokcoin git repository](https://github.com/youngseokcoin/youngseokcoin) but are not part of
...
Currently only `Source10` is used, for the example `youngseokcoin.conf` file.
...
in the youngseokcoin git repository. At present only an SVG version of the Youngseokcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the youngseokcoin
...
needed when 0.12.1 is released, a proper fix is already in the Youngseokcoin git
master branch.
...
idea to build Youngseokcoin Core against a static version of that library compiled
...
The `youngseokcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `youngseokcoin-server` package. At this point, most RPM based
...
The `youngseokcoin-server` package should have SELinux support. How to properly do
...
Youngseokcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/youngseokcoin/). He has been
packaging Youngseokcoin for Fedora at least since 2012.
...
4. I renamed the `youngseokcoin` package that contains the Qt GUI to `youngseokcoin-core` as
that appears to be how the general population refers to it, in contrast to
`youngseokcoin-xt` or `youngseokcoin-classic`. I wanted to make sure the general population
